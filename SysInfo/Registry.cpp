...
// Defines for retrieving last user who'd been logged in on Win10
#define WIN10_LASTLOGGEDUSER_USER_KEY			_T( "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon")
#define WIN10_LASTLOGGEDUSER_USER_VALUE			_T( "LastUsedUsername" )
...
BOOL CRegistry::GetUser(CString &csUser) {
	HKEY m_hKey;
	TCHAR value[255];
	DWORD value_length = 255;
	DWORD type = REG_SZ;

	if (RegOpenKeyEx(m_hKey, WIN10_LASTLOGGEDUSER_USER_KEY, 0, KEY_READ | KEY_WOW64_64KEY, &m_hKey) == ERROR_SUCCESS)
	{
		// Get user name.
		if (GetValue(m_hKey, WIN10_LASTLOGGEDUSER_USER_VALUE, csUser) == ERROR_SUCCESS)
		{
			RegCloseKey(m_hKey);
			AddLog(_T("\t\t<User: %s>\n\tOK\n"), csUser);
			return TRUE;
		...
		AddLog(_T("\tFailed in call to <RegQueryValueEx> function for HKCU\\%s\\%s !\n"),
			...
			WIN10_LASTLOGGEDUSER_USER_VALUE);
		RegCloseKey(m_hKey);
	}
	else
		AddLog(_T("\tFailed in call to <RegOpenKey> function for HKCU\\%s !\n"),
