...
set OCSAGENT_PATH=C:\Users\devel\Documents\GitHub\WindowsAgent32
...
echo.
cd /d "%OPENSSL_PATH%"
perl Configure VC-WIN32 no-asm --prefix=%OPENSSL_PATH%\out32
nmake clean
nmake
nmake install
if ERRORLEVEL 1 goto ERROR
copy out32\lib\libssl.lib %EXT_DEPS%
copy out32\lib\libcrypto.lib %EXT_DEPS%
copy out32\bin\libcrypto-1_1.dll %OCSAGENT_PATH%\Release
copy out32\bin\libssl-1_1.dll %OCSAGENT_PATH%\Release

set INCLUDE=%OPENSSL_PATH%\out32\include;%ZLIB_PATH%;%INCLUDE%
set LIB=%OPENSSL_PATH%\out32\lib;%ZLIB_PATH%;%LIB%
...
nmake clean
nmake /f Makefile.vc WINBUILD_ACKNOWLEDGE_DEPRECATED=yes mode=dll VC=17 ENABLE_SSPI=no ENABLE_IDN=no ENBALE_IPV6=yes WITH_SSL=dll WITH_ZLIB=dll WITH_DEVEL=%OPENSSL_PATH%\out32 SSL_LIBS="libcrypto.lib libssl.lib"
if ERRORLEVEL 1 goto ERROR
copy ..\builds\libcurl-vc17-x86-release-dll-ssl-dll-zlib-dll-ipv6\lib\libcurl.lib %EXT_DEPS%
copy ..\builds\libcurl-vc17-x86-release-dll-ssl-dll-zlib-dll-ipv6\bin\libcurl.dll %OCSAGENT_PATH%\Release
if ERRORLEVEL 1 goto ERROR
